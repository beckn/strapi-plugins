...
export default ({ strapi }: { strapi: Strapi }) => ({
  async createConsumptionLog({ customerId }) {
    try {
      ...
      const currentMeterReading = previousMeterReading + unitConsumed;
      ...
      const newLog = await strapi.entityService.create(
        "api::consumption-log.consumption-log",
        {
          data: {
            ...
            current_meter_reading: currentMeterReading.toString(),
  ...
  async createProductionLog({ customerId }) {
    try {
      ...
      const currentMeterReading = previousMeterReading + unitProduced;
