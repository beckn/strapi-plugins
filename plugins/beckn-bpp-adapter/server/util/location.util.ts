...
// Decode polyline
function decodePolyline(encodedPolyline: any) {
  return polyline.decode(encodedPolyline).map((pair: any) => ({
    latitude: pair[0],
    longitude: pair[1]
  }));
}
...
function isStoreNearRouteWithinDistance(
  store: any,
  routePoints: any,
  location: any,
  maxDistance: any
  ...
  if (maxDistance && location) {
    pointsWithinDistance = routePoints.filter((point: any) =>
      geolib.isPointWithinRadius(point, location, maxDistance)
    );
  } else {
...
export const findStoresAlongRouteWithinDistance = (
  ...
  const storesAlongRoute = stores.filter((store: any) =>
