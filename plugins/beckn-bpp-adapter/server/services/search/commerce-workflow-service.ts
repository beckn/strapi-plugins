...
export default ({ strapi }: { strapi: Strapi }) => ({
  async index({ message, context }: { message: any; context: any }) {
    ...
    await Promise.all(
      providers.map(async (provider: any) => {
        await Promise.all(
          await provider.items.map(async (item: any) => {
            await Promise.all(
    ...
    if (item?.tags?.length && item?.tags[0]?.list?.length) {
      const newProviders = providers.map((provider: any) => {
        const filteredItems = provider?.items?.filter((itemFromStrapi: any) => {
