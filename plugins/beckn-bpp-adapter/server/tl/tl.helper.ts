...
export const quote = async (items: KeyValuePair[]) => {
  ...
  items.map((item) => {
    item?.sc_retail_product?.price_bareakup_ids?.map((price_bareakup_id: KeyValuePair) => {
      breakup.push({
...
export const payments = async (
  ...
  if (!incomingPrice) {
    const priceValue = provider?.items.reduce((accumulator: number, currentValue: KeyValuePair) => accumulator + Number(currentValue?.sc_retail_product.min_price), 0);
