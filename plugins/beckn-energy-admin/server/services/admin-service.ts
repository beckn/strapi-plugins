...
export default ({ strapi }: { strapi: Strapi }) => ({
  ...
  async updateMarketStatus(status) {
    try {
      if (status !== 'OPEN' && status !== 'CLOSED') {
        throw new Error('Invalid status provided to update market status');
      }
      //market-status is a single-type, so it will always have only one row with id = 1
      const marketStatus = await strapi.entityService.update("api::market-status.market-status", 1,
        {
          data: {
            status,
            publishedAt: new Date()
